...
public class Practice13GetTextBoundsView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        charHeight = rect.bottom - rect.top;
        offset = (height - charHeight)/2;
        canvas.drawText(text1, 100, bottom - offset, paint2);

        paint2.getTextBounds(text2,0,1,rect);
        charHeight = rect.bottom - rect.top;
        offset = (height - charHeight)/2;
        canvas.drawText(text2, 200, bottom - offset, paint2);

        paint2.getTextBounds(text3,0,1,rect);
        charHeight = rect.bottom - rect.top;
        offset = (height - charHeight)/2;
        canvas.drawText(text3, 300, bottom - offset, paint2);

        paint2.getTextBounds(text4,0,1,rect);
        charHeight = rect.bottom - rect.top;
        offset = (height - charHeight)/2;
        canvas.drawText(text4, 400, (top + bottom)/2+ 1.5f * offset, paint2);

        paint2.getTextBounds(text5,0,1,rect);
        charHeight = rect.bottom - rect.top;
        offset = (height - charHeight)/2;
        canvas.drawText(text5, 500, bottom - offset, paint2);

        paint2.getTextBounds(text6,0,1,rect);
        charHeight = rect.bottom - rect.top;
        offset = (height - charHeight)/2;
        canvas.drawText(text6, 600, bottom - offset, paint2);
    }
}